...
const EditTeachers = () => {
    ...
    useEffect(() => {
        axios.get(`http://${process.env.REACT_APP_SERVER_NAME}:8000/users/teachers/${params.id}`, {
            headers: {
                "x-access-token": token
            }
        }).then((response) => {
            if (response.data.message === "Not found Teacher") {
            ...
            } else if (response.data.message === "Error retrieving Teacher") {
                toast.error('Error retrieving Teacher!', {
                    position: "top-right",
                    autoClose: 1000,
                    hideProgressBar: false,
                    closeOnClick: true,
                ...
                setFormValues({
                    ...
                    newPassword: {
                        value: "",
                        error: false,
                        errorMessage: 'You must enter an new password'
                    },
                })
            }
        }).catch((error) => {
    ...
    const updateStudent = () => {
        axios.put(`http://${process.env.REACT_APP_SERVER_NAME}:8000/users/teachers/update/${params.id}`, {
    ...
    const updateNotify = () => toast.success('Teacher updated successfully!', {
        position: "top-right",
        autoClose: 1000,
        hideProgressBar: false,
        closeOnClick: true,
